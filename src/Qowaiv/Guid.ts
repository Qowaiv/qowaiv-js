  ...
  export class Guid implements IEquatable, IFormattable, IJsonStringifyable {
    ...
    public static newGuid(seed?: Guid): Guid {
      ...
      if (seed !== null && seed instanceof Guid) {
        ...
        let merged = '';
        for (let i = 0; i < 36; i++) {
          const l = lookup.indexOf(seed.v.charAt(i));
          const r = lookup.indexOf(guid.v.charAt(i));
    ...
    private static rnd(): string {
      ...
      return Guid.rndGuid() + Guid.rndGuid() + Guid.rndGuid() + Guid.rndGuid();
    }
    ...
    public version(): number {
      return parseInt(this.v.substr(14, 1), 10);
    }
